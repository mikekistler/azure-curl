...
groupName=$(tail -1 listPrivateLinks.out | jq -r '.value[0].name')

# -s hides the progress bar
# -D sends the headers to stdout
curl -s -D - -X GET \
   -H "Authorization: Bearer ${token}" \
   ${baseUrl}/configurationStores/${configStoreName}/privateLinkResources/${groupName}?api-version=${apiVersion} \
   > getPrivateLink.out

head -n 1 getPrivateLink.out
tail -n 1 getPrivateLink.out | jq '.'
