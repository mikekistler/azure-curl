...
curl -s -D - -X GET \
   ...
   ${baseUrl}/configurationStores/${configStoreName}/privateEndpointConnections?api-version=${apiVersion} \
   > listPrivateEndpoints.out

head -n 1 listPrivateEndpoints.out
tail -n 1 listPrivateEndpoints.out | jq '.'
