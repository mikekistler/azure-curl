...
curl -s -D - -X PUT \
   ...
   -d "${body}" \
   ${baseUrl}/configurationStores/${configStoreName}/privateEndpointConnections/${privateEndpointConnectionName}?api-version=${apiVersion} \
   > createPrivateEndpoint.out

head -n 1 createPrivateEndpoint.out
tail -n 1 createPrivateEndpoint.out | jq '.'
